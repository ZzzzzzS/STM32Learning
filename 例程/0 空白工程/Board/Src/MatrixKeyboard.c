...
char KeyClicked(void) {
  ...
  GPIO_ResetBits(GPIOD, GPIO_Pin_9 | GPIO_Pin_8);  // 设置为低电平
  if (GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_13) == 1) {
    Delay_ms(10);                                        //按键消抖
    if (GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_13) == 1)  //确定按键被按下
      key = 11;
  } else if (GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_12) == 1) {
    Delay_ms(10);                                        //按键消抖

if (GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_12) == 1)  //确定按键被按下
      key = 21;
  } else if (GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_11) == 1) {
    Delay_ms(10);                                        //按键消抖
    if (GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_11) == 1)  //确定按键被按下
      key = 31;
  } else if (GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_10) == 1) {
    Delay_ms(10);                                        //按键消抖
    if (GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_10) == 1)  //确定按键被按下
      key = 41;
  }

  /******操作第二列按键******/
  GPIO_SetBits(GPIOB, GPIO_Pin_15);                // 设置为高电平
  GPIO_ResetBits(GPIOD, GPIO_Pin_8 | GPIO_Pin_9);  // 设置为低电平
  ...
  GPIO_ResetBits(GPIOB, GPIO_Pin_14 | GPIO_Pin_15);  //设置为低电平
  if (GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_13) == 1) {
    Delay_ms(10);  //按键消抖                                   //按键消抖
    if (GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_13) == 1)  //确定按键被按下
      key = 14;

...
unsigned char row[] = {0xef, 0xdf, 0xbf, 0x7f, 0xfe, 0xfd, 0xFB, 0xf7};
...
void 	Dispaly_M(uint32_t zhengshu,uint32_t xiaoshu)//数码管显示函数
{
	int i=0,a[4];
			while(1)
			{
		
				if(xiaoshu>=1000&&xiaoshu<=9999)
					break;
					xiaoshu=xiaoshu/10;
			}
			a[0]=xiaoshu/100/10;
		...
		while(1)
		{	
			DisplayScan(5,data_table[15]);
			DisplayScan(6,data_table[15]);
			DisplayScan(7,data_table[15]);
			for(i=0;i<=3;i++)
			DisplayScan(i,data_table[a[i]]);
		}	
}
